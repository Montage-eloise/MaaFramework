...
int main()
{
    ...
    auto json_text = ReadFileToString("./niu.json");

    auto task_id = MaaTaskerPostTask(tasker_handle, "AutoAttack", json_text.c_str());
    ...
    void* trans_arg)
{
    ...
    if (pt) {
        std::cout << "Current point: (" << pt->x << ", " << pt->y << ")"
                  << " Click point: " << pt->x * image.cols / 800 << ", " << pt->y * image.rows / 600 << std::endl;
